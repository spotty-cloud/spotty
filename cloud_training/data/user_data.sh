...
# sync the project
aws --profile cloud_training s3 sync "{{s3_project_dir}}" "/data/{{project_name}}" --exclude "*" --include "{{package_name}}/*" --include "data/*.zip" --include "training/*/last_session"
...
echo "aws --profile cloud_training s3 sync /data/{{project_name}}/training/{{model_name}} {{s3_project_dir}}/training/{{model_name}}" > sync.sh
chmod +x sync.sh
(crontab -l ; echo "*/5 * * * * ~/sync.sh > /dev/null 2>&1") | crontab - > /dev/null
...
# start training
PYTHONPATH=/data/{{project_name}} nohup python /data/{{project_name}}/{{package_name}}/models/{{model_name}}/train.py -s {{session_id}} > train.log 2>&1 &
